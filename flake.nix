{
  ...
  outputs = { self, flake-utils, naersk, nixpkgs, rust-overlay }:
    flake-utils.lib.eachDefaultSystem (system:
      ...
      in rec {
        # `nix build`
        packages.kv-store-ninja = naersk-lib.buildPackage {
          pname = "kv-store-ninja";
          root = ./.;
        };
        defaultPackage = packages.kv-store-ninja;

        # `nix run` or `nix run .#app`
        apps.app = flake-utils.lib.mkApp {
          drv = packages.kv-store-ninja;
        };
        defaultApp = apps.app;
